...
export interface EstatisticasVeiculo {
  ...
  kgAnoAtual: number;
  /** Kg carregados por mês do ano corrente, Jan–Dez (0 nos meses sem carga). */
...
export async function carregarEstatisticasVeiculo(veiculoId: number): Promise<EstatisticasVeiculo> {
  const paragens = await prisma.paragem.findMany({
    where: { veiculoId },
    select: { cliente: true, kgCarregados: true, data: true },
  });
  ...
  const kgAnoAtual = doAno.reduce((a, p) => a + p.kgCarregados, 0);

  const porMes = new Array(12).fill(0);
  for (const p of doAno) porMes[p.data.getMonth()] += p.kgCarregados;
