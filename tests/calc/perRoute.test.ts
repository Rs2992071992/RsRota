...
// Rateio separado por segmento (Ida / Volta): cada segmento como uma rota à
// parte. Só ativa quando há entregas/recolhas mesmo marcadas "Volta".
describe("calcularRota — rateio por segmento Ida/Volta", () => {
  // Rota-teste RIC-Blo-greenopinion: 3 entregas na ida + VAZIO + 1 recolha
  // grande na volta (casimper). Palete PALETE_120X80, capacidade 38.
  const pal = (over: Partial<ParagemInput>) =>
    paragemBase({ idRota: "SEG01", volume: true, tipoPalete: "PALETE_120X80", ...over });
  const rota: ParagemInput[] = [
    pal({ cliente: "greenopinion", tipoViagem: "Ida", kmInicial: 0, kmFinal: 120, nPaletes: 4 }),
    pal({ cliente: "plastiagro", tipoViagem: "Ida", kmInicial: 120, kmFinal: 240, nPaletes: 5 }),
    pal({ cliente: "Carvidet", tipoViagem: "Ida", kmInicial: 240, kmFinal: 360, nPaletes: 13 }),
    paragemBase({ idRota: "SEG01", cliente: "Vazio", tipoVeiculo: "VAZIO", tipoViagem: "Volta", kmInicial: 360, kmFinal: 440 }),
    pal({ cliente: "casimper", tipoViagem: "Volta", kmInicial: 440, kmFinal: 540, nPaletes: 22 }),
  ];
  const r = calcularRota("SEG01", rota, ctx);

  it("Σ custo atribuído = custo total da rota; Σ quotas = 1", () => {
    expect(r.rateio.reduce((a, c) => a + c.custoAtribuido, 0)).toBeCloseTo(r.custoTotalRota, 4);
    expect(r.rateio.reduce((a, c) => a + c.quota, 0)).toBeCloseTo(1, 6);
  });

  it("rateioPorSegmento tem os 2 segmentos e Σ dos seus custos = custo total", () => {
    expect(r.rateioPorSegmento).toBeDefined();
    expect(r.rateioPorSegmento!.map((s) => s.segmento).sort()).toEqual(["Ida", "Volta"]);
    const soma = r.rateioPorSegmento!.reduce((a, s) => a + s.custo, 0);
    expect(soma).toBeCloseTo(r.custoTotalRota, 4);
  });

  it("casimper (volta, sozinho) deixa de pagar ~50 % — paga só o seu segmento", () => {
    const casimper = r.rateio.find((c) => c.cliente === "casimper")!;
    // No modelo antigo casimper = 22/44 = 50 %. Agora só a volta (troço + ½ vazio).
    expect(casimper.quota).toBeLessThan(0.4);
    const volta = r.rateioPorSegmento!.find((s) => s.segmento === "Volta")!;
    expect(volta.clientes).toHaveLength(1);
    expect(volta.clientes[0].cliente).toBe("casimper");
    expect(casimper.custoAtribuido).toBeCloseTo(volta.custo, 4);
  });

  it("os 3 clientes da ida dividem o segmento da ida por 4:5:13", () => {
    const ida = r.rateioPorSegmento!.find((s) => s.segmento === "Ida")!;
    const get = (n: string) => ida.clientes.find((c) => c.cliente === n)!.custoAtribuido;
    expect(get("plastiagro") / get("greenopinion")).toBeCloseTo(5 / 4, 3);
    expect(get("Carvidet") / get("greenopinion")).toBeCloseTo(13 / 4, 3);
  });

  it("o vazio reparte-se 50 % ida / 50 % volta", () => {
    const semVazio = calcularRota(
      "SEG02",
      rota.filter((p) => p.tipoVeiculo !== "VAZIO"),
      ctx,
    );
    const custoVazio = r.custoTotalRota - semVazio.custoTotalRota;
    const voltaCom = r.rateioPorSegmento!.find((s) => s.segmento === "Volta")!.custo;
    const voltaSem = semVazio.rateioPorSegmento!.find((s) => s.segmento === "Volta")!.custo;
    expect(voltaCom - voltaSem).toBeCloseTo(custoVazio / 2, 3);
  });

  it("rota só com Ida -> sem rateioPorSegmento, resultado igual ao modelo plano", () => {
    const soIda = calcularRota(
      "SEG03",
      rota.filter((p) => p.tipoVeiculo !== "VAZIO").map((p) => ({ ...p, tipoViagem: "Ida" })),
      ctx,
    );
    expect(soIda.rateioPorSegmento).toBeUndefined();
    const soma = soIda.rateio.reduce((a, c) => a + c.custoAtribuido, 0);
    expect(soma).toBeCloseTo(soIda.custoTotalRota, 4);
    // casimper agora com 22/44 do total (modelo plano)
    const casimper = soIda.rateio.find((c) => c.cliente === "casimper")!;
    expect(casimper.quota).toBeCloseTo(22 / 44, 3);
  });

  it("custos de nível de rota (horas extra) repartem-se por ida + volta", () => {
    const comHoras = calcularRota(
      "SEG04",
      rota.map((p, i) => (i === 0 ? { ...p, horasExtra: 3 } : p)),
      ctx,
    );
    const casSem = r.rateio.find((c) => c.cliente === "casimper")!.custoAtribuido;
    const casCom = comHoras.rateio.find((c) => c.cliente === "casimper")!.custoAtribuido;
    // casimper está só na volta mas apanha parte das horas extra (registadas na ida)
    expect(casCom).toBeGreaterThan(casSem);
    expect(comHoras.rateio.reduce((a, c) => a + c.custoAtribuido, 0)).toBeCloseTo(
      comHoras.custoTotalRota,
      4,
    );
  });

  it("recolha na volta faturada a um cliente da ida -> aparece nos 2 segmentos, rateio fundido soma", () => {
    const comRecolha = calcularRota(
      "SEG05",
      [
        ...rota,
        pal({ cliente: "Fornecedor", tipoViagem: "Volta", kmInicial: 540, kmFinal: 600, nPaletes: 3, faturarCliente: "greenopinion" }),
      ],
      ctx,
    );
    const idaG = comRecolha.rateioPorSegmento!.find((s) => s.segmento === "Ida")!.clientes.find((c) => c.cliente === "greenopinion");
    const voltaG = comRecolha.rateioPorSegmento!.find((s) => s.segmento === "Volta")!.clientes.find((c) => c.cliente === "greenopinion");
    expect(idaG).toBeDefined();
    expect(voltaG).toBeDefined();
    const fundidoG = comRecolha.rateio.find((c) => c.cliente === "greenopinion")!;
    expect(fundidoG.custoAtribuido).toBeCloseTo(idaG!.custoAtribuido + voltaG!.custoAtribuido, 4);
    expect(comRecolha.rateio.find((c) => c.cliente === "Fornecedor")).toBeUndefined();
    expect(comRecolha.rateio.reduce((a, c) => a + c.custoAtribuido, 0)).toBeCloseTo(comRecolha.custoTotalRota, 4);
  });
});
